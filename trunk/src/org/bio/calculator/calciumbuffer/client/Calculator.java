...
import org.bio.calculator.calciumbuffer.client.species.*;
...
public class Calculator extends Composite {
	...
	public static Metal H  = new Metal("H" , 1);
	public static Metal Li = new Metal("Li", 1);
	public static Metal Be = new Metal("Be", 2);
	public static Metal Na = new Metal("Na", 1);
	public static Metal Mg = new Metal("Mg", 2);
	public static Metal K  = new Metal("K" , 1);
	public static Metal Ca = new Metal("Ca", 2);
	public static Metal Rb = new Metal("Rb", 1);
	public static Metal Sr = new Metal("Sr", 2);
	public static Metal Cs = new Metal("Cs", 1);
	public static Metal Ba = new Metal("Ba", 2);
	public static Metal Fr = new Metal("Fr", 1);
	public static Metal Ra = new Metal("Ra", 2);
	public static Species Cl = new Anion("Cl", 1);
	...
	void onBtnCalculateClick(ClickEvent event) {
		...
		//TODO:  define solutes, update & ISC for solutes, add to buffer, iterate, report all in buffersolution lists
		...
		BufferingAgent TES = new BufferingAgent("TES", 7.4, true); // args are buffer name, pKa, cationic
		
		//
		// normally all ingredients should be added via drop-downs with text boxes for concentrations and buttons for free/total
		//
		
		BufferSolution myBuffSol = new BufferSolution(7.1, .160, 25.0);   // args are pH, ionic strength, temp
		
		myBuffSol.add(new MetalSolute( myBuffSol, Ca  , 0.000000010, Solute.State.free ));
		myBuffSol.add(new MetalSolute( myBuffSol, Sr  , 0.000001   , Solute.State.free ));
		myBuffSol.add(new MetalSolute( myBuffSol, Mg  , 0.00316228 , Solute.State.free )); 
		myBuffSol.add(new LigandSolute(myBuffSol, ATP , 0.00316228 , Solute.State.free ));
		myBuffSol.add(new LigandSolute(myBuffSol, EGTA, 0.005	   , Solute.State.total ));
		myBuffSol.add(new LigandSolute(myBuffSol, HDTA, 0.005	   , Solute.State.total ));
		myBuffSol.add(new LigandSolute(myBuffSol, CP  , 0.012	   , Solute.State.total ));
		myBuffSol.add(new BufferSolute(myBuffSol, TES , 0.300						    ));
		
		if (!myBuffSol.Iterate(.000001, 100000)) //args are convergence limit, max # iterations
		{
			pnlResult.add(new Label("D'OH!"));
		}
		else
		{
			/* 
